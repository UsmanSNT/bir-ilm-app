...
import { ArrowRight, Bell, BookOpen, Bookmark, CalendarDays, ChartNoAxesColumnIncreasing, ChevronLeft, ChevronRight, CircleHelp, Globe, Heart, Info, Mail, MessageCircle, NotebookPen, Settings, ShieldCheck, Sparkles, Trophy, UserRound, Users } from "lucide-react";
...
type Screen = "profile" | "activity" | "settings" | "posts" | "messages" | "privacy" | "faq" | "about";
...
export default function ProfileScreens(p: Props) {
  ...
  const titles: Record<Screen, string> = { profile: "Shaxsiy sahifa", activity: "Faollik", settings: "Sozlamalar", posts: "Mening postlarim", messages: "Xabarlar", privacy: "Maxfiylik va xavfsizlik", faq: "Ko‘p so‘raladigan savollar", about: "Bir Ilm haqida" };
  ...
  return <section className={`profile-space p-view-${screen}`}>
    ...
    {screen === "settings" && <div className="p-settings">
      ...
      <h2>Hisob</h2><div className="p-menu"><Row icon={<UserRound />} title="Shaxsiy ma’lumotlar" onClick={p.onEdit} /><Row icon={<ShieldCheck />} title="Maxfiylik va xavfsizlik" onClick={() => open("privacy")} /></div>
